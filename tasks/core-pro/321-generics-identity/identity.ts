...
export class IdentityProcessor<T extends Identity> {
    constructor(private readonly platform: T['provider']) { }

    findById(id: string): T | undefined {
        return users.find((user): user is T => user.id === id && user.provider === this.platform)
    }

    findByUserName(userName: string): T | undefined {
        return users.find((user): user is T => user.userName === userName && user.provider === this.platform)
    }
}
